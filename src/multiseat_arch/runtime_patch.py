...
def install(backend: ModuleType) -> None:
    ...
    The target CachyOS host uses the patched wlroots/Labwc stack as the actual
    compositor for each DRM-leased connector. A full Plasma session cannot be
    layered on top because it would start KWin and compete for the DRM device,
    so Labwc launches an LXQt Wayland session as its desktop shell instead.
    """
    ...
    def start_seat(config, seat) -> None:
        ...
        # PAMName=login/pam_systemd establishes /run/user/<uid>. Labwc then
        # publishes WAYLAND_DISPLAY and DISPLAY (XWayland) to the startup shell,
        # which is critical for Steam and legacy X11 applications.
        env = {
            "XDG_SEAT": runtime,
