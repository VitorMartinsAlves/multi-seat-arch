...
def _prepare_pcmanfm_profile() -> None:
    """Give the Labwc/LXQt seat a visible desktop and preserve Plasma wallpaper."""
...
def main() -> int:
    # A full Plasma session would start KWin, which cannot use the patched
    # wlroots DRM_LEASE path. LXQt is officially supported on Labwc and provides
    # the Qt desktop/panel/menu while keeping Labwc as the seat compositor.
    ...
    # Last-resort fallback keeps the seat usable if LXQt packages were removed.
